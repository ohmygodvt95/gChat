    ...
    replies.each do |message_id|
      ...
      if message
        ...
        self.content = self.content.gsub "@reply:#{message.id}",
          "<span data=\"#{message.id}\" class=\"reply\"><img src=\"#{message.user.avatar.thumb.url}\"/> #{I18n.t("rooms.re")}</span>"
      end
    end
    ...
    mentions.each do |user_id|
      ...
      if user
        ...
        self.content = self.content.gsub "@mention:#{user.id}",
          "<span data=\"#{user.id}\" class=\"mention\"><img src=\"#{user.avatar.thumb.url}\"/> #{I18n.t("rooms.me")}</span>"
      end
    end
    self.save
  end
